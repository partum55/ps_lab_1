...
top_words_by_class %>%
  ...
  coord_flip() +
  labs(
    title = "Top 15 Words/Tokens in Ham vs Spam (Preprocessed Data)",
    x = "Word/Token",
    y = "Frequency"
  ) +
  theme_minimal() +
...
```{r word-cloud}
regular_words <- word_counts %>%
  filter(!word %in% c("email", "url", "phone", "money", "longnum"))

ham_words <- subset(regular_words, label == "ham")
spam_words <- subset(regular_words, label == "spam")
...
title("Ham Messages", cex.main = 1.2)
...
title("Spam Messages", cex.main = 1.2)
...
### 1. Acсuracy Comparison Bar Plot

```{r fig.width=14, fig.height=8}
# Prepare data for plotting with shorter labels
plot_data <- comparison_data
plot_data$Short_Label <- c("Baseline\n(Lemma+NoStop)", "Stemming\n(+Stopwords)", "Bigrams\n(Lemma+Stop)", "Trigrams\n(Lemma+Stop)", "No Special\n(Lemma+Stop)")
plot_data$Short_Label <- factor(plot_data$Short_Label, levels = plot_data$Short_Label)

ggplot(plot_data, aes(x = Short_Label, y = Accuracy, fill = Short_Label)) +
...
# Add shorter labels for better readability
comparison_data_short <- comparison_data
comparison_data_short$Short_Label <- c("Baseline", "Stemming", "Bigrams", "Trigrams", "No Special")

metrics_long <- comparison_data_short %>%
  select(
    Short_Label, SPAM_Precision, SPAM_Recall, SPAM_F1,
    HAM_Precision, HAM_Recall, HAM_F1
  ) %>%
  pivot_longer(cols = -Short_Label, names_to = "Metric", values_to = "Value") %>%
  separate(Metric, into = c("Class", "Metric_Type"), sep = "_")

ggplot(metrics_long, aes(x = Short_Label, y = Value, fill = Metric_Type)) +
...
**Cons:** Strong independence assumption (words are independent), sensitive to unstable data, bad performance with unseen word combinations in test data.

**Accuracy vs F1-Score:** Accuracy can be misleading with imbalanced datasets (For example 95% ham, 5% spam). A model predicting "all ham" would have 95% accuracy but 0% spam detection. F1-score balances precision and recall, providing better insight into performance on minority classes, which is crucial for spam detection where missing spam (false negatives) is costly.
