...
install.packages(c("readr", "stringi", "tokenizers", "SnowballC", "textstem"))
```
...
naiveBayes <- setRefClass("naiveBayes",
  ...
  methods = list(
    ...
    score = function(X_test, y_test) {
      ...
      predictions <- sapply(X_test, function(msg) .self$predict(msg))
      
      actuals <- as.character(y_test)
      predictions <- as.character(predictions)
      
      correct_predictions <- sum(predictions == actuals)
      total_predictions <- length(actuals)
      accuracy <- correct_predictions / total_predictions

      confusion_mat <- table(Actual = actuals, Predicted = predictions)
...
```{r run-usage-eval}
test_results <- model$score(X_test = test_data$Message, y_test = test_data$Category)

print(paste("Test Accuracy:", round(test_results$accuracy, 4)))
