...
wordcloud(words = ham_words$word,
          freq = ham_words$n,
          min.freq = 5,
          max.words = 100,
          scale = c(3, 0.5),
...
wordcloud(words = spam_words$word,
          freq = spam_words$n,
          min.freq = 5,
          max.words = 100,
          scale = c(3, 0.5),
...
```{r}
# Display compact results table with better formatting
compact_results <- data.frame(
  Configuration = c("1. Baseline", "2. Stemming", "3. Bigrams", "4. Trigrams", "5. No Special"),
  Accuracy = paste0(round(comparison_data$Accuracy * 100, 1), "%"),
  Predictions = paste0(comparison_data$Correct, "/", comparison_data$Total),
  SPAM_F1 = round(comparison_data$SPAM_F1, 3),
  HAM_F1 = round(comparison_data$HAM_F1, 3)
)
compact_results
```
...
# Create plots for all confusion matrices
...
# Arrange plots in a grid (3x2 for 5 experiments)
...
**Mathematical Foundation:** We implemented a Multinomial Naive Bayes classifier based on Bayes' theorem: P(class|message) ∝ P(class) × ∏P(word|class). The method uses Laplace smoothing and log-probabilities to avoid numerical underflow.

**Pros:** Simple, fast, works well with small datasets, handles high-dimensional sparse data effectively, probabilistic output.

**Cons:** Strong independence assumption (words are independent), sensitive to skewed data, poor performance with unseen word combinations in test data.
